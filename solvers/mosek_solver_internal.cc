...
namespace {
// A 1x1 scalar-valued psd constraint "X is psd" is the same as a bound on the
// variable X(0, 0) >= 0. We update `lb` and `ub` on the variables with X(0, 0)
// >= 0.
  ...
  for (const auto& binding : psd_constraints) {
    if (binding.evaluator()->matrix_rows() == 1) {
      ...
      switch (bound_dual.first.type) {
        ...
        default: {
          throw std::runtime_error(
              "The dual variable for a scalar PositiveSemidefiniteConstraint "
              "lower bound can "
              "only be slx or slc.");
        }
      }
